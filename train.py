...
from Loss import WeightedBCELoss  # Import the custom loss function
...
weight_bona_fide = config['loss']['weight_bona_fide']
weight_spoofed = config['loss']['weight_spoofed']
runType = 'Train'
...
criterion = WeightedBCELoss(weight_bona_fide, weight_spoofed)
optimizer = optim.Adam(model.parameters(), lr=lr)

# Define the paths to the data
flac_train_folder =  r'ASV_Data\Dataset200\Dataset200_train\flac' # Path to the training data folder
train_labels = 'ASV_Data\Dataset200_train.txt' # Path to the training labels file
flac_val_folder = r'ASV_Data\Dataset200\Dataset200_val\flac' # Path to the validation data folder
val_labels = 'ASV_Data\Dataset200_val.txt' # Path to the validation labels file

deg = 47
imp = np.zeros((129, deg + 1))  # Will be initialized once for all instances
for k in range(deg + 1):
    imp[:, k] = np.flipud(np.loadtxt(f'0.885_0.985_1.00_129/legendre_0.885_0.985_1.00_129_{k}.txt')) / 1e15

# Initialize the dataset and dataloaders
train_dataset = AudioDataset(flac_train_folder, train_labels, deg, imp, device, runType)
val_dataset = AudioDataset(flac_val_folder, val_labels, deg, imp, device, runType)  # For validation, you can define a separate folder for val data if needed
...
checkpoint_dir = "checkpoints\DatasetTrain"
...
plot_dir = "checkpoints\DatasetTrain"
...
for epoch in range(start_epoch, epochs):
    ...
    if min_tDCF <= min_tDCF_best:
        ...
        checkpoint_path = f"checkpoints\DatasetTrain/best_epoch_{epoch+1}.pth"
        torch.save({
            'epoch': epoch + 1,
    ...
    checkpoint_path = f"checkpoints\DatasetTrain/epoch_{epoch+1}.pth"
    torch.save({
        'epoch': epoch + 1,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
    ...
    if (epoch + 1) % 10 == 0:
        ...
        # Save the plot
        plot_path = os.path.join(plot_dir, f"loss_curve.png")
